...
AS
   WITH LAST_EXTRACT_DATE_TBL
        AS (SELECT NVL (STG.LAST_EXTRACT_DATE,
     ...
     AS (SELECT /*+ parallel (16)*/
          ...
          WHERE     ITM.BUSCOMP_NAME = 'LOY Membership Card'
                ...
                AND (ITM.CREATED + 7 / 24 >
                        (SELECT LAST_EXTRACT_DATE FROM LAST_EXTRACT_DATE_TBL))
                ---------------------
                -- Check New Value --
                ---------------------
                AND (   (    SUBSTR (TO_CHAR (ITM.AUDIT_LOG),
                                     ...
                                     - INSTR (TO_CHAR (ITM.AUDIT_LOG),
                                              ...
                                              1,
                                              3)
                                     - 2) = 'STATUS_CD' -- Case Active Flag + Status
                         AND SUBSTR (TO_CHAR (ITM.AUDIT_LOG),
                                       INSTR (TO_CHAR (ITM.AUDIT_LOG),
                                              '*',
                                              1,
                                     ...
                                     - INSTR (TO_CHAR (ITM.AUDIT_LOG),
                                              ...
                                              1,
                                              6)
                                     - 2) = 'Active') -- Check New Value= 'Active'
                     OR (    SUBSTR (TO_CHAR (ITM.AUDIT_LOG),
                                       INSTR (TO_CHAR (ITM.AUDIT_LOG),
                                              '*',
                                              1,
                                              2)
